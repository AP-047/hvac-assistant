...
# Default for local development, override with Azure Web App URL in production
...
def retrieve_chunks(query: str, top_k: int = 3) -> List[Dict[str, Any]]:
    ...
    Returns empty list if services are unavailable.
    """
    # Check if services are available
    if not client or not embedder:
        logger.warning("Qdrant client or embedder unavailable, returning empty results")
        return []
